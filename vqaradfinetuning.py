...
finetuning.py
-------------
Finetune a vision-language model (SmolVLM) on VQA-RAD using LoRA and 8-bit quantization with DDP.
"""
...
def collate_fn(batch, processor):
    ...
    answers = [ex["answer"] for ex in batch]  # single answer (not list)
...
def load_model(model_id, local_rank):
    ...
    # Move to GPU *before* LoRA and DDP
...
def compute_perplexity(dataset, model, processor, batch_size):
    ...
    loader = DataLoader(
        dataset,
        batch_size=1,  # Force batch_size=1 for per-example ppl
        shuffle=False,
...
def compute_sample_exposures(dataset, model, processor, top_k=5):
    ...
    for ex in tqdm(dataset, desc="Computing exposures", disable=(dist.get_rank() != 0)):
        img = ex["image"].convert("RGB")
        q = ex["question"]
        prompt = f"<image> {q}"

        true_ans = ex.get("answer", "").lower()
        if not true_ans:
        ...
        hit = sum(1 for g in gen_ans if g in true_ans or true_ans in g)
...
def finetune():
    ...
    try:
        ...
        # ----------------------------
        print(f"[Rank {rank}] Loading VQA-RAD...")
        train_ds = load_dataset("flaviagiammarino/vqa-rad", split="train")
        val_ds = load_dataset("flaviagiammarino/vqa-rad", split="test")
